...
# 데이터 변환
convert_yolo_dataset_to_classification('data_for_learn/train', 'data_for_learn_cls/train')
convert_yolo_dataset_to_classification('data_for_learn/valid', 'data_for_learn_cls/val')
...
# 데이터셋 로드
image_datasets = {
    'train': datasets.ImageFolder('data_for_learn_cls/train', transform=transform['train']),
    'val': datasets.ImageFolder('data_for_learn_cls/val', transform=transform['val'])
}
data_loaders = {
...
# 모델 저장
os.makedirs('model_v3/cropped', exist_ok=True)
torch.save(model.state_dict(), 'model_v3/resnet50_model.pth')
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('model_v3/train_accuracy.png')
plt.close()
...
print(report)
with open('model_v3/classification_report.txt', 'w') as f:
    f.write(report)