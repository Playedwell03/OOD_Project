...
# --- (수정됨) create_and_add_ood_from_class0 함수 ---
def create_and_add_ood_from_class0(base_dataset_dir, ood_dataset_dir, output_base_dir, ood_ratio=0.5, seed=42):
    ...
    # (기존 코드와 동일)
    split_train = 'train' # 'train' 스플릿에 대해서만 OOD 작업을 수행합니다.
    ...
    if not class0_ood_files:
        ...
        # OOD 파일이 없어도 valid/test 복사는 수행해야 하므로 return 대신 경고만 합니다.
    ...
    for split_name in ['valid', 'test']:
        ...
        if os.path.isdir(src_dir):
            # 대상 디렉토리가 이미 있으면 삭제 (shutil.copytree를 안전하게 사용하기 위해)
    ...
    print("✅ 'valid'/'test' 폴더 복사 완료.")

# --- 사용 예시 ---
# 아래 경로들을 실제 환경에 맞게 수정하여 사용하세요.
create_and_add_ood_from_class0(
    base_dataset_dir='A_k10_runs/run_10_test',              # 원본 데이터셋 폴더 (train/valid/test 포함)
    ood_dataset_dir='fish_class_0',     # 클래스 0 데이터를 가져올 OOD 데이터셋 폴더
    output_base_dir='A_k10_runs_10per/run_10_test',  # 결과물을 저장할 폴더
    ood_ratio=0.1                          # 최종 'train' 스플릿에서 OOD가 차지할 비율 (0.3 = 30%)
)